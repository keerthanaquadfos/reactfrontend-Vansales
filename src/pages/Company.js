 ...
 const handleDelete = ()=>{
  ...
  return( 
    <>
    {
     ...
     <Layout>
     <div className="card">
                <h5 className="card-header"><i className="bx bx-plus-circle me-2" style={{cursor:'pointer'}} onClick={()=>modalShow({id:0})}></i>Companies</h5>
                ...
                comapnies.map((data,index) => {
                      if(data.id>1){
                        return(<tr key={data.id}>  
                           <td>{data.name}</td>
                           <td>{data.address.length>80 ? data.address.substring(0,80):data.address}</td>
                           <td>{data.contact}</td>
                           <td>{data.email}</td>
      ...
      <Modal show={show} onHide={modalClose}> 
        ...
        <InputType className={'form-control'} inputType={'password'} labelFor={'password'} 
          onChange={(e)=>setPassword(e.target.value)}
          labelText={'Admin Password'} name={password} placeholder={'Admin Password'} value={password}/>    
        
        <InputType className={'form-control'} inputType={'number'} labelFor={'maxUsers'} 
          onChange={(e)=>setMaxUsers(e.target.value)}
          labelText={'Max Users'}  placeholder={'Max Users'} value={maxUsers}/>  
