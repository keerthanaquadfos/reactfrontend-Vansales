...
const Country = () => {
  ...
  const postCategory = async () =>{
    try{
      if(!code){
        toast.warning("Country code required");
        return;
      }else  if(!name){
 ...
 const handleDelete = ()=>{
  ...
  return( 
    <>
    {
     ...
     <Layout>
     <div className="card">
                <h5 className="card-header"><i className="bx bx-plus-circle me-2" style={{cursor:'pointer'}} onClick={()=>modalShow({id:0,name:"",code:""})}></i>Countries</h5>
      ...
      <Modal show={show} onHide={modalClose}> 
        ...
        <Modal.Body>
          ...
          labelText={'Country Code'} name={code} placeholder={'Country Code'} value={code}/>
           <InputType className={'form-control'} inputType={'text'} labelFor={'name'} 
          onChange={(e)=>setName(e.target.value)}
          labelText={'Country Name'} name={name} placeholder={'Country Name'} value={name}/>
        </Modal.Body>
        <Modal.Footer>
