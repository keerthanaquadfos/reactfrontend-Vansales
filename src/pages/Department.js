...
const Department = () => {
  ...
  const getDepartments = async ()=> {
    try{
        setLoading(true);     
        const {data} = await API.get('/department', null) 
        console.log(data);
        if(data.status){  
 ...
 const handleDelete = ()=>{
  ...
  return( 
    <>
    {
     ...
     <Layout>
     <div className="card">
                <h5 className="card-header"><i className="bx bx-plus-circle me-2" style={{cursor:'pointer'}} onClick={()=>modalShow({id:0})}></i>Department</h5>
      ...
      <Modal show={show} onHide={modalClose}> 
        ...
        <Modal.Body> 
          ...
          labelText={'Department Name'} name={name} placeholder={'Department Name'} value={name}/>
        </Modal.Body>
        <Modal.Footer>
